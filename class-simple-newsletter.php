...
if ( ! class_exists( 'Simple_Newsletter_Widget' ) ) :
	...
	class Simple_Newsletter_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			add_action( 'wp_enqueue_scripts', array( &$this, 'add_jQuery_libraries' ), 10 );
		}
		...
		function newsletter_ajax_request( $instance ) {
			...
			if ( isset($_REQUEST) ) {
				...
				// grab the api key, and the last part after hyphen is usually the correct data center
		...
		// Registering/Enqueue Scripts
		function add_jQuery_libraries() {
			// add a check to see if its already been enqueued before enqueueing scripts
			wp_enqueue_script( 'jquery-validation-plugin', 'http://ajax.aspnetcdn.com/ajax/jquery.validate/1.11.1/jquery.validate.min.js', array('jquery'), '', true );
		}
	...
	if ( !function_exists('simple_newsletter_shortcode') ) {
		...
		function simple_newsletter_shortcode( $atts ) {
			...
			// $output .= new Simple_Newsletter_Widget; // need to find a way to repurpose code above for shortcode, maybe even plugin settings
